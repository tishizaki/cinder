...
class SheepdogClientTestCase(test.TestCase):
    ...
    def test_parse_location_none(self):
        ...
        self.assertEqual('Image None is unacceptable: Image location is NULL.',
                         exc.msg)
    ...
    def test_get_volume_stats_success(self, fake_execute):
        ...
        actual = self.client.get_volume_stats()
        fake_execute.assert_called_once_with(*expected_cmd)
    ...
    @mock.patch.object(sheepdog.SheepdogClient, '_run_dog')
    @mock.patch.object(sheepdog, 'LOG')
    def test_get_volume_stats_fail_to_get_node_info(self, fake_logger,
                                                    fake_execute):
        cmd = self.test_data.CMD_DOG_NODE_INFO
        exit_code = 2
        stdout = 'stdout_dummy'
    ...
    @mock.patch.object(sheepdog.SheepdogClient, '_run_dog')
    @mock.patch.object(sheepdog, 'LOG')
    def test_get_volume_stats_parse_stdout_error(self, fake_logger,
                                                 fake_execute):
        stdout = 'stdout_dummy'
        stderr = ''
        fake_execute.return_value = (stdout, stderr)
        fake_execute.side_effect = None
        self.assertRaises(AttributeError, self.client.get_volume_stats)
        self.assertTrue(fake_logger.exception.called)
