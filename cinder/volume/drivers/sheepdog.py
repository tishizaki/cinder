...
class SheepdogIOWrapper(io.RawIOBase):
    ...
    def read(self, length=None):
        if not self._is_valid():
            msg = _('An error for reading an invalid volume "%s" happens.')
            raise exception.VolumeDriverException(msg % self._vdiname)
    ...
    def write(self, data):
        if not self._is_valid():
            msg = _('An error for writing to an invalid volume "%s" happens.')
            raise exception.VolumeDriverException(msg % self._vdiname)
    ...
    def seek(self, offset, whence=0):
        if not self._is_valid():
            msg = _('An error for seeking an invalid volume "%s" happens.')
            raise exception.VolumeDriverException(msg % self._vdiname)
...
class SheepdogDriver(driver.VolumeDriver):
    ...
    def backup_volume(self, context, backup, backup_service):
        ...
        # NOTE(tishizaki)
        # If previous backup_volume operation has failed,
